  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6" role="region" aria-label="Resumo do Inventário">
      ...
      <StatCard 
        ...
        colorClass="border-red-500 from-red-900 to-black animate-pulse shadow-[0_0_15px_rgba(239,68,68,0.4)]"
...
const StatCard: React.FC<{
  ...
  valueColor?: string;
  description?: string;
}> = ({ label, value, icon, colorClass, accentColor, valueColor = "text-yellow-300", description }) => (
  <article 
    className={`bg-gradient-to-br ${colorClass} border-b-4 rounded-xl p-5 shadow-2xl transform transition-all hover:scale-[1.02] hover:-translate-y-1 duration-300 relative overflow-hidden group`}
    aria-label={`${label}: ${value}`}
  >
    <div className="absolute -right-4 -bottom-4 opacity-10 group-hover:scale-125 transition-transform duration-500" aria-hidden="true">
      {icon}
    </div>
    <div className="flex justify-between items-start relative z-10">
      <div>
        <p className="text-white/60 text-[10px] font-black uppercase tracking-[0.2em] mb-1" aria-hidden="true">{label}</p>
        <h3 className={`text-5xl font-black ${valueColor} drop-shadow-md`}>
          <span className="sr-only">{label} de </span>
          {value}
        </h3>
        {description && <p className="sr-only">{description}</p>}
      </div>
      <div className={`${accentColor} p-2 bg-black/20 rounded-lg backdrop-blur-sm`} aria-hidden="true">
        {icon}
      </div>
    </div>
  </article>
